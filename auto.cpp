...
{ 
  ...
  Eigen::Matrix4f user_transform_matrix;
  pcl::visualization::PCLVisualizer::convertToEigenMatrix (init_transform, user_transform_matrix);

  pcl::transformPointCloud (*source, *source, user_transform_matrix);
  /*       preprocessing        */
  //esitmate normal
  ...
  CloudNormalPtr source_sampled_normal (new CloudNormal);
  CloudNormalPtr target_sampled_normal (new CloudNormal);
  ne.setInputCloud (source_sampled);
  ne.compute(*source_sampled_normal);
  //  *target_normal = *source_normal;
  ne.setInputCloud (target_sampled);
  ne.compute(*target_sampled_normal);
  ...
  sift_detect.setInputCloud (source_sampled);
  pcl::PointCloud<pcl::PointWithScale> temp_source_keypoints;
  sift_detect.compute (temp_source_keypoints);
  CloudRGBPtr source_keypoints  (new CloudRGB);
  pcl::copyPointCloud (temp_source_keypoints, *source_keypoints);

  sift_detect.setInputCloud (target_sampled);
  pcl::PointCloud<pcl::PointWithScale> temp_target_keypoints;
  sift_detect.compute (temp_target_keypoints);
  CloudRGBPtr target_keypoints  (new CloudRGB);
  pcl::copyPointCloud (temp_target_keypoints, *target_keypoints);
  printf ("target keypoints %zu, source keypoints %zu\n", target_keypoints->points.size (), source_keypoints->points.size ());
  /* fpfh descriptors*/
  pcl::FPFHEstimation<PointRGB, pcl::Normal, pcl::FPFHSignature33> fpfh_estimation;
  ...
  fpfh_estimation.setSearchSurface (source_sampled);  
  fpfh_estimation.setInputNormals (source_sampled_normal);
  fpfh_estimation.setInputCloud (source_keypoints);
  pcl::PointCloud<pcl::FPFHSignature33>::Ptr source_descriptors (new pcl::PointCloud<pcl::FPFHSignature33>);
  fpfh_estimation.compute (*source_descriptors);
  
  fpfh_estimation.setSearchSurface (target_sampled);
  fpfh_estimation.setInputNormals (target_sampled_normal);
  fpfh_estimation.setInputCloud (target_keypoints);
  pcl::PointCloud<pcl::FPFHSignature33>::Ptr target_descriptors (new pcl::PointCloud<pcl::FPFHSignature33>);
  fpfh_estimation.compute (*target_descriptors);

  printf ("target descriptor: %zu, source descriptor: %zu\n", target_descriptors->points.size (), source_descriptors->points.size ());

  /*init sac_ia*/

  pcl::SampleConsensusInitialAlignment<PointRGB, PointRGB, pcl::FPFHSignature33> sac_ia;
  ...
  CloudRGB registration_output;
  sac_ia.align (registration_output);
  Eigen::Matrix4f init_transform_matrix = sac_ia.getFinalTransformation ();
  /*-------------------*/
  pcl::transformPointCloud (*source_sampled, *source_sampled, init_transform_matrix);
  ...
  error[0] = compute_error (target, source, Eigen::Matrix4f::Identity ());
